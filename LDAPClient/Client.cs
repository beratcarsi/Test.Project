...
namespace LDAPClient
{
    ...
    public class Client
    {
        ...
        public Client(string username, string domain, string password, string url)
        {
            ...
            //using (var connection = new LdapConnection(new LdapDirectoryIdentifier(url)))
            //{
            ...
            const string _domain = "192.168.1.23";
            const string _constrParts = @"OU=Users,DC=elpo,DC=local";
            const string _username = @"berat@elpo.local";
            const string _password = "123123";
            //var principalContext = new PrincipalContext(ContextType.Domain, _domain, _constrParts);
            var principalContext = new PrincipalContext(ContextType.Domain, _domain, _constrParts, ContextOptions.SimpleBind, _username, _password);
            var userPrincipal = UserPrincipal.FindByIdentity(principalContext, _username);
